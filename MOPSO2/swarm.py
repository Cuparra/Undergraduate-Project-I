...
class Particles:
    ...
    def __init__(self,swarm):
        ...
        self.fitness     = np.zeros( (sizeofSwarm , sizeOfFitness), np.float64)
        self.best        = np.zeros( (sizeofSwarm,sizeOfDimension),np.float64 )
        self.wost        = np.zeros( (sizeofSwarm,sizeOfDimension),np.float64 )
        self.velocity    = np.zeros( (sizeofSwarm,sizeOfDimension), np.float64)
        self.position    = np.random.rand(sizeofSwarm,sizeOfDimension)*(h_num-l_num) + l_num
