...
def problem1(fit,x):
    ...
    x[x>1] = 1
    x[x<-1] = -1
    var    = np.zeros((50,15),np.float64)

    for i in range(50):
        for j in range(15):
            var[i][j] = x[i][0] = (x[i][0] + 1.0)/2.0

    fit[:,0] =  x[:,0] + (2.0/15)*sum(( x[:,2::2] - sin(6.0*pi*var[:,1:] + (1.0*J1[:,2::2]*pi/30) ))**2 ,axis = 1)
    fit[:,1] = 1 - x[:,0]**0.5 + (2.0/15)*sum(( x[:,1::2] - sin(6.0*pi*var[:,1:] + (1.0*J1[:,1::2]*pi/30) ))**2 ,axis = 1)
